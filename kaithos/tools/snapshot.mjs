...
import { ORDER_COSTS, FACTIONS, GOLDEN_ANGLE, SCALE, CENTER, N_PROVINCES } from '../src/config.js';
...
if (days > 0) {
  ...
  for (let d = 0; d < days && !s.over; d++) {
    ...
    // train the thin garrisons
    for (const p of mine) {
      if (me.ap >= 1 && me.gold >= 5 && p.garrison < 9) { me.ap -= 1; me.gold -= 5; s.orders.push({ kind: 'train', prov: p.i }); }
    }
    // strike inward with the strongest holding
    ...
    if (strong && me.ap >= 2) {
      ...
      if (t) { me.ap -= 2; s.orders.push({ kind: 'march', from: strong.i, to: t.i, troops: strong.garrison - 1, mode: 'shadow', cost: ORDER_COSTS.march }); }
    }
    h.onAdvance();
  }
}
